...
const Orders = (props) => {
  ...
  const orderItem = async (formFields) => {
    ...
    formData.append(
      "senderLocationDetails",
      JSON.stringify(senderLocationDetails)
    );
    formData.append(
      "recepientLocationDetails",
      JSON.stringify(recepientLocationDetails)
    );
  ...
  const OrderSchema = Yup.object().shape({
    ...
    productWeight: Yup.number()
      ...
      .test(
        "maxLength",
        "mininum weight should be " + selectedCat.minWeight,
        (val, allFormFields) => {
          const selectedFieldMinWeight = categories.find(
            (item) => item.categoryName === allFormFields.parent.productType
          ).minWeight;
              ...
              props.item
                ? {
                    ...
                    expectedDeliveryDate: dayjs(
                      props.item.expectedDeliveryDate
                    ),
            ...
            {({
              ...
              <Form onSubmit={handleSubmit}>
                ...
                {errors.productWeight && touched.productWeight ? (
                  <div className="error">{errors.productWeight}</div>
                ) : null}
                ...
                <Field
                  ...
                  onBlur={handleBlur}
                />
                {errors.senderLocation && touched.senderLocation ? (
                  <div className="error">{errors.senderLocation}</div>
                ) : null}

                <Field
                  name="receipentLocation"
                  placeholder="Receipent Location"
                  onChange={handleChange}
                  onBlur={handleBlur}
                />
                {errors.receipentLocation && touched.receipentLocation ? (
                  <div className="error">{errors.receipentLocation}</div>
                ) : null}
                {currentDistance ? (
                  <h5>The distance is {currentDistance} KM</h5>
                ) : (
                  ""
                )}
                ...
                <Field
                  ...
                  onBlur={handleBlur}
                />
                {errors.receipentName && touched.receipentName ? (
                  <div className="error">{errors.receipentName}</div>
                ) : null}
                ...
                />
                {errors.expectedDeliveryDate && touched.expectedDeliveryDate ? (
                  <div className="error">{errors.expectedDeliveryDate}</div>
                ) : null}

                <select
                  name="expectedDeliveryTime"
                  value={values.expectedDeliveryTime}
                  onChange={handleChange}
                  onBlur={handleBlur}
                >
                  <option
                ...
                <FileUploader
                  handleChange={saveFile}
                  type="file"
                  types={fileTypes}
                />

                {errors.expectedDeliveryTime && touched.expectedDeliveryTime ? (
                  <div className="error">{errors.expectedDeliveryTime}</div>
                ) : null}
                <button type="submit">
                  {!props.isEdit ? "Send" : "Edit"} order
                </button>
              </Form>
            )}
          </Formik>
