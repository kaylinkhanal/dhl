...
import { setCurrentDistance, setRecepientLocationDetails, setRecepientAddress } from "../../reducers/locationSlice"
      ...
      dragend() {
        ...
        if (marker != null) {
          ...
          fetch(`https://api.geoapify.com/v1/geocode/reverse?lat=${markerRef.current.getLatLng().lat}&lon=${markerRef.current.getLatLng().lng}&apiKey=a1dd45a7dfc54f55a44b69d125722fcb`)
            .then((res) => res.json())
            .then((data => dispatch(setRecepientAddress(data?.features[0]?.properties?.formatted))))
        }
      },
    }),
